...
public:
    ...
    rstd::mutex _mutexItems; ///< protects _listRemoveItems
    mutable rstd::mutex _mutexGUIFunctions;
    mutable rstd::condition _notifyGUIFunctionComplete; ///< signaled when a blocking _listGUIFunctions has been completed
    //@}
    ...
    rstd::mutex _mutexUpdating; ///< when inside an update function, even if just checking if the viewer should be updated, this will be locked.
    rstd::mutex _mutexUpdateModels; ///< locked when osg environment is being updated from the underlying openrave environment
    ...
    //@{ callbacks
    rstd::mutex _mutexCallbacks; ///< maintains lock on list of callsbacks viewer has to make like _listRegisteredViewerThreadCallbacks, _listRegisteredItemSelectionCallbacks
