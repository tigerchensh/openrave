...
QtCoinViewer::~QtCoinViewer()
{
    ...
    {
        rstd::unique_lock<rstd::mutex> lock(_mutexMessages);
...
{
    boost::shared_ptr<rstd::unique_lock<rstd::mutex>> lock = boost::make_shared<rstd::unique_lock<rstd::mutex>>(_mutexGUI);
...
void QtCoinViewer::SetEnvironmentSync(bool bUpdate)
{
    ...
    if( !bUpdate ) {
        ...
        rstd::unique_lock<rstd::mutex> lockmsg(_mutexMessages);
...
void QtCoinViewer::AdvanceFrame(bool bForward)
{
    ...
    //    {
    //        _bInIdleThread = true;
    //        rstd::unique_lock<rstd::mutex> lock(_mutexGUI);
    //        _bInIdleThread = false;
    //    }
    ...
    : _pviewer(pviewer), _ppreturn(ppreturn)
{
    // get a mutex
    if( bWaitForMutex ) {
        _plock = boost::make_shared<rstd::unique_lock<rstd::mutex>>(_mutex);
    }
}
...
void QtCoinViewer::EnvMessage::callerexecute(bool bGuiThread)
{
    ...
    if( bGuiThread ) {
    ...
    else {
        {
            QtCoinViewerPtr pviewer = _pviewer.lock();
            if( !!pviewer ) {
                rstd::unique_lock<rstd::mutex> lock(pviewer->_mutexMessages);
...
bool QtCoinViewer::_SaveBodyLinkToVRMLCommand(ostream& sout, istream& sinput)
{
    ...
    rstd::unique_lock<rstd::mutex> lock(_mutexUpdateModels);
    rstd::unique_lock<rstd::mutex> lock2(g_mutexsoqt);
