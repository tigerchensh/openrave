...
QtCoinViewer::~QtCoinViewer()
{
    ...
    {
        boost::unique_lock<boost::mutex> lock(_mutexMessages);
...
void QtCoinViewer::SetEnvironmentSync(bool bUpdate)
{
    ...
    if( !bUpdate ) {
        ...
        boost::unique_lock<boost::mutex> lockmsg(_mutexMessages);
...
void QtCoinViewer::AdvanceFrame(bool bForward)
{
    ...
    //    {
    //        _bInIdleThread = true;
    //        boost::unique_lock<boost::mutex> lock(_mutexGUI);
    //        _bInIdleThread = false;
    //    }
...
void QtCoinViewer::EnvMessage::callerexecute(bool bGuiThread)
{
    ...
    if( bGuiThread ) {
    ...
    else {
        {
            QtCoinViewerPtr pviewer = _pviewer.lock();
            if( !!pviewer ) {
                boost::unique_lock<boost::mutex> lock(pviewer->_mutexMessages);
...
bool QtCoinViewer::_SaveBodyLinkToVRMLCommand(ostream& sout, istream& sinput)
{
    ...
    boost::unique_lock<boost::mutex> lock(_mutexUpdateModels);
    boost::unique_lock<boost::mutex> lock2(g_mutexsoqt);
