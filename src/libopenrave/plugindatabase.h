...
class RaveDatabase : public boost::enable_shared_from_this<RaveDatabase>
{
    struct RegisteredInterface : public UserData
    {
        ...
        virtual ~RegisteredInterface() {
            ...
            if( !!database ) {
                rstd::unique_lock<rstd::mutex> lock(database->_mutex);
...
protected:
        ...
        rstd::mutex _mutex;         ///< locked when library is getting updated, only used when plibrary==NULL
        rstd::condition _cond;
    ...
    {
        _threadPluginLoader.reset(new rstd::thread(boost::bind(&RaveDatabase::_PluginLoaderThread, this)));
    ...
    virtual void Destroy()
    {
        ...
        {
            rstd::unique_lock<rstd::mutex> lock(_mutexPluginLoader);
            _bShutdown = true;
...
protected:
    ...
    void _PluginLoaderThread()
    {
        while(!_bShutdown) {
            ...
            FOREACH(itplugin,listPluginsToLoad) {
                ...
                rstd::unique_lock<rstd::mutex> lockplugin((*itplugin)->_mutex);
                if( _bShutdown ) {
                    break;
                }
    ...
    mutable rstd::mutex _mutex;     ///< changing plugin database
    ...
    /// \name plugin loading
    //@{
    mutable rstd::mutex _mutexPluginLoader;     ///< specifically for loading shared objects
    rstd::condition _condLoaderHasWork;
    std::list<PluginPtr> _listPluginsToLoad;
    boost::shared_ptr<rstd::thread> _threadPluginLoader;
    bool _bShutdown;
    //@}
};
