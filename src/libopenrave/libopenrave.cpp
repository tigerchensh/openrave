...
static rstd::once_flag _onceRaveInitialize = BOOST_ONCE_INIT;
...
public:
    ...
    {
        rstd::call_once(_create,_onceRaveInitialize);
        return _state;
    }
    ...
    void Destroy()
    {
        ...
        {
            rstd::unique_lock<rstd::mutex> lock(_mutexinternal);
...
public:
        ...
        {
            rstd::unique_lock<rstd::mutex> lock(global->_mutexinternal);
...
//        return 0;
    ...
    rstd::mutex& GetInitializationMutex() {
...
private:
    ...
    int _nDebugLevel;
    rstd::mutex _mutexInitialization; ///< external mutex for initialization only
    rstd::mutex _mutexinternal;
...
int RaveInitialize(bool bLoadAllPlugins, int level)
{
    ...
    rstd::unique_lock<rstd::mutex> lock(state->GetInitializationMutex());
    if( state->_IsInitialized() ) {
        RAVELOG_WARN_FORMAT("[th:%s] OpenRAVE already initialized, so not initializing again", rstd::this_thread::get_id());
        return 0;
    }
    else {
...
{
    if( !RaveGlobalState() ) {
        RAVELOG_WARN_FORMAT("[th:%s] OpenRAVE global state is not initialized! Need to call RaveInitialize before any OpenRAVE services can be used. For now, initializing with default parameters.", rstd::this_thread::get_id());
        uint64_t starttime = utils::GetMicroTime();
        RaveInitialize(true);
        RAVELOG_WARN_FORMAT("[th:%s] OpenRAVE global state finished initializing in %u[us].", rstd::this_thread::get_id()%(utils::GetMicroTime()-starttime));
    }
